...
class NORMAL(nn.Module):
    def __init__(self, config):
        ...
        for i in range(self.depth): # 2
            ...
            self.decoders.append(_Decoder(decoder_filter_config[i], # + cat_feature[i],
    ...
    def forward(self, x):
        ...
        for i in range(self.depth):
            feat = self.decoders[i](feat, indices[self.depth -1 - i], unpool_sizes[self.depth -1 - i])
            # if i =! 0:
            feat = F.dropout(feat, p=self.drop_rate)
...
class _Decoder(nn.Module):
    ...
    def forward(self, x, indices, size):
        ...
        # print("unpooled",unpooled.size())
        # print("cat_feature",cat_feature.size())
        # cat_feature = torch.cat((unpooled,cat_feature),dim=1)
        # print("cat_feature",cat_feature.size())
